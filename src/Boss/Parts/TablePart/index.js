...
export default class TablePart extends Component {
  ...
  render() {
    ...
    const tableBoxStyle = Object.assign({}, styles.contener, { maxHeight });

    if (!items || !items.length || items.length === 0) {
      paginationStyle.display = 'none';
    }
    const fixTable = operationsFix ? (<Table hoverable center style={styles.fixdRight} >
      {this.getFixHeader()}
      {this.genFixRows()}
    </Table>) : '';
    return (
          ...
          >
            {
              ...
              :
                <div style={tableBoxStyle}>
                  <Table
                    hoverable center style={minWidth ? { minWidth } : {}}
